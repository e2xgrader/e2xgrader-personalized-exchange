...
import nbgrader.exchange.default.submit as default_submit
...
from .submit_utils import generate_student_info_file, generate_submission_html
...
class ExchangeSubmit(Exchange, default_submit.ExchangeSubmit):
    ...
    exam_mode = Bool(
        ...
        help=dedent(
            """
            A flag whether the current user is in exam mode. Used for generating exam files.
            """
        ),
    ).tag(config=True)
