...
import nbgrader.exchange.default.collect as default_collect
...
class ExchangeCollect(Exchange, default_collect.ExchangeCollect):
    update = Bool(
    ...
    def init_submissions(self):
        ...
        if self.personalized_inbound:
            ...
            for user in submit_dirs:
                ...
                usergroups.update(
                    default_collect.groupby(user_records, lambda item: item["username"])
                )
            ...
            usergroups = default_collect.groupby(records, lambda item: item["username"])
